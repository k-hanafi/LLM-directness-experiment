"""Merge per-batch CSVs into the active arm's final classified CSV.

Output path:
  outputs/arm_a/classified_arm_a.csv
  outputs/arm_b/classified_arm_b.csv

These two files plus the imported v2 baseline are the three inputs to
the directness analysis.
"""
...
from src.context import batch_outputs_dir, merged_csv
...
def merge_batch_csvs(
    ...
    """Concatenate all per-batch CSVs into one final output for the active arm."""
    output_path = output_path or merged_csv()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fieldnames = list(ClassificationResult.model_fields.keys())
    total_rows = 0

    in_dir = batch_outputs_dir()
...
def _build_cost_table(state: PipelineState) -> Table:
    ...
    table = Table(title=f"Pipeline Cost Report (arm={state.arm})", expand=True)
    table.add_column("Metric", style="bold")
    table.add_column("Value", justify="right")

    table.add_row("Model", state.model)
    table.add_row("Arm", state.arm)
...
def print_report(state: PipelineState, output_path: Path | None = None) -> None:
    """Print the full classification distribution and cost report for one arm."""
    output_path = output_path or merged_csv()
    console = Console()
    ...
    logger.info(
        "Report (arm=%s): %d companies classified, %d batches, cache hit %.1f%%",
        state.arm,
        len(rows),
